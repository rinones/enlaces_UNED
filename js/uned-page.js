...
export function initUnedPage(){
  ...
  function renderSelector(){
    if(selectorEl && selectorEl.children && selectorEl.children.length > 0){
      ...
      existingBtns.forEach(btn=>{ btn.replaceWith(btn.cloneNode(true)); });
      const buttons = selectorEl.querySelectorAll('[data-subject]');
      buttons.forEach(btn=>{ const subj = (btn.getAttribute('data-subject')||'').trim().toLowerCase(); btn.addEventListener('click', ()=>{ setSelected(subj); }); });
      buttons.forEach(btn=>{ const subj = (btn.getAttribute('data-subject')||'').trim().toLowerCase(); const isSel = !!selected && subj===selected; if(isSel){ btn.classList.add('active'); btn.setAttribute('aria-pressed','true'); } else { btn.classList.remove('active'); btn.setAttribute('aria-pressed','false'); } });
      updateIndicators();
      return;
    }
  ...
  function loadAllUpcoming(){
    ...
    return Promise.all(slugs.map(s=>fetchJSON(`data/activities/subjects/${s}.json`).then(a=>[s, Array.isArray(a)?a:[]]))).then(pairs=>{
      ...
      pairs.forEach(([s,a])=>{ a.forEach(x=>{ const d=parseYMD(x.date||x.ymd); if(!d) return; 
